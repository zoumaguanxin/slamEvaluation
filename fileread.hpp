...
namespace evaluation
{
  ...
  void readfile(const char* completeDir, storeT & vec)
  {
    ifstream file;
    file.open(completeDir);
    if(file.good())
    {
      file>>vec;
    }
    ...
    {
      cout<<"can't open the file in the specified diretory, please check whether the diretory is set corrcetly"<<endl;
      assert(file.good());
    }    
  }  
